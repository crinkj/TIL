> # SOLID(객체지향 설계 5원칙)
> ## 개요 
* SOLID 원칙은 `객체지향적`인 디자인이다. SOLID 원칙은 개발자들이 유지보수성이 뛰어나고,가독성이 좋고, 유연성있는 소프트웨어를 개발하기 위해 필요한 5가지의 원칙이다. 어플리케이션이 확장되고 커질수록 복잡도는 증가한다, 개발자는 SOLID원칙을 통해 복잡도를 감소시켜 후에 유지보수하기 좋아지며 탄탄한 어플리케이션을 만들 수 있다. 
> ## Single Responsibility Principal(SRP) - 단일 책임 원칙
...
> ## Open-Closed Principal(OCP, Open for Extension,Closed For Modification) - 개방 폐쇄 원칙
...
> ## Liskov Substitution Principal(LSP) - 리스코프 치환 원칙
...
> ## Interface Segregation Principal(ISP) - 인터페이스 분리 원칙
...
> ## Dependency Inversion Principal(DIP) - 의존 역전 원칙
* Dependency Inversion Principal은 소프트웨어 모듈간의 결합도를 감소시킨다. DIP 원칙은 결합도를 줄이기 위해 상위 모듈이 하위모듈에게 의존하면안되고 추상적인것에 의존해야한다. 쉽게말하면 구현체에 의존하기보단 추상적인것에 의존해야한다.
```
...
```
* 위 클래스를 살펴보면, Window98Machine이라는 클래스가 생성될때마다 Monitor와 standardKeyBoard가 딸려온다. 이 경우 Windows98Machine 클래스는 Monitor(), StandardKeyboard() 두 클래스와 결합도가 강하게 묶여있다. 이럴 경우 만약 사용자가 다른 모니터 다른 키보드로 대체하고 싶을 경우 수정을 하면 기존 코드에도 변경이 필요하고DIP원칙에 위배된다.

```
(1) public interface Keyboard { }

(2) public class StandardKeyboard implements Keyboard { }
...
```
* 위와같이 keyboard를 `interface` 로 분리하고 기존있던 `StandardKeyboard` 클래스를 `Keyboard` 인터페이스로 상속받고 구현한다.
위와같이 변경하면 `keyboard` 라는 추상적인것을 통해 구현되기때문에 클래스들간에 결합도는 감소된다. 또한 다른 키보드로 변경을 원하면 기존 Window98Machine 클래스는 수정하지않고 
원하는 종류의 키보드 클래스를 만든 후 `keyboard` 인터페이스를 상속받아 구현해서 변경하기 편하다.

> ## 결론
* 좋은 객체지향적인 개발은 `다형성(상속을 통해 기능을 확장하거나 변경하는것)`을 잘 사용하는것이다. `SOLID` 의 원칙을 따르면 유지보수하기 쉽고 유연성이 뛰어난 소프트웨어를 개발할 수 있다.
유지보수하기 쉽고 유연성이 뛰어난 코드란 좋은 코드이며 잘짜인 코드를 뜻한다. 좋은 개발자가 되기위해선 좋은 코드를 짜야한다. 그러므로 객체지향적으로 개발을 할 떄 `SOLID`의 원칙을 잘따라야 좋은개발자,
좋은 코드를 작성할 수 있다.
> ## 참조
